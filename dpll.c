...
void dpll_Update()
{
	...
	if (dpll.intr[0] == 0) // проверка наличия сигнала на входе
	{
		/* Свипирование */
		dpll.T0++;
		if(dpll.T0 > (DPLL_T_MIN >> 1))
			dpll.T0++;
		if(dpll.T0 > (DPLL_T_MIN >> 2))
			dpll.T0++;
	...
	MDR_TIMER1->ARR = dpll.T; // Обновление основания счета

	MDR_TIMER1->CCR1 = (MDR_TIMER1->ARR >> 1) + (MDR_TIMER1->ARR & 0x1); // Обновление регистра захвата CCR1
	MDR_TIMER1->CCR11 = MDR_TIMER1->ARR; // Обновление регистра захвата CCR11

#if SCH_TYPE == 2

	MDR_TIMER1->CCR2 = MDR_TIMER1->CCR1 - (preset->att * MDR_TIMER1->ARR)/360; // Обновление регистра захвата CCR2
	MDR_TIMER1->CCR21 = MDR_TIMER1->CCR11 - (preset->att * MDR_TIMER1->ARR)/360; // Обновление регистра захвата CCR21
#endif
}
